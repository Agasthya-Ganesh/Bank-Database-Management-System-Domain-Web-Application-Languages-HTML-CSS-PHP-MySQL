...
<head>
    ...
    define('bankName', 'KBC Bank',true);
		...
		if (isset($_POST['cashierLogin']))
		{
		    ...
		    $result = $con->query("select * from cas_login where email='$user' AND password='$pass'");
		    if($result->num_rows>0)
		    { 
		      session_start();
		      $data = $result->fetch_assoc();
		      $_SESSION['cashId']=$data['id'];
			  $_SESSION['active_cashier']=$data['name'];
		      //$_SESSION['user'] = $data;
		...
		if (isset($_POST['managerLogin']))
		{
		    ...
		    $result = $con->query("select * from mgr_login where email='$user' AND password='$pass'");
		    if($result->num_rows>0)
		    { 
		      session_start();
		      $data = $result->fetch_assoc();
		      $_SESSION['managerId']=$data['id'];
			  $_SESSION['active_manager']=$data['name'];
		      //$_SESSION['user'] = $data;
...
<h1 class="alert alert-success rounded-0"><img src="images/banklogo.png" width="60" height="60" class="d-inline-block align-top" alt=""><?php echo bankname; ?><small class="float-right text-muted" style="font-size: 12pt;"><kbd>Presented by:Agasthya Ganesh G</kbd></small></h1>
<br>
<?php echo $error ?>
<br>
